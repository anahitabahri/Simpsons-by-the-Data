...
Next steps: word plots for top 10 characters (possibly incorporate rsentiment package), avg imdb rating per season with layer on us viewers in millions (need to transform to stay on same axis), how many times does homer say "Doh!"?, wordcount per episode (or season) over time for non-simpson family characters, % of show's total dialogue by season for simpsons fam, where do the top 10 characters speak most (split by character), who is usually the first to talk in the episode? 